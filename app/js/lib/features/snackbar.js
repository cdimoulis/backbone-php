this.Snackbar = function(options) {
	...
	this.message = function(message, opts) {
		...
		var $snackbar = this._addSnackbar(message, opts);
		this._fadeIn($snackbar);

		if (!opts.manual_close) {
			setTimeout(function() {
				_this._removeSnackbar($snackbar);
			}, opts.time);
		}
	}
	...
	// opt: options to send
	this._addSnackbar = function(message, opts) {
		var _this = this;
	...
	//Remove a snackbar
	this._removeSnackbar = function($el) {
		this._fadeOut($el, function() {
			$el.remove();
		});
	};

	//Fade in individual snackbar
	this._fadeIn = function($el) {
		$el.animate({opacity: 1}, 500);
	};

	//Fade out individual snackbar
	this._fadeOut = function($el, end) {
